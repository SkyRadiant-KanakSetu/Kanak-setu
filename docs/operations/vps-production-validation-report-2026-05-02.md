...
**Latest operator outcome (reference):** `APP_DIR=/opt/kanak-setu bash scripts/prod/post-deploy-verify.sh` → health checks PASS, backup freshness PASS, telemetry summary **HEALTHY**, `last-verify.json` present.
